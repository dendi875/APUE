/**
 * 该程序证明如果main函数调用了用户函数，用户函数调用返回到main函数，main函数再返回到启动例程，
 *　启动例程再用main的返回值，调用exit，exit再调用注册到atexit的函数，隐匿调用_exit或_Exit返回
 *　内核
 */
#include "apue.h"
...
/*实验：
[dendi875@localhost Chapter-7-Process Environment]$ ./my_exit4
main is done
first exit handler
first exit handler
second exit handler
[dendi875@localhost Chapter-7-Process Environment]$ echo $?
1
